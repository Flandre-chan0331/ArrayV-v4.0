 ...
 *
 */
public final class OptimizedMidSingularityQuickSort extends Sort {

    public OptimizedMidSingularityQuickSort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Optimized Mid Singularity Quick");
        this.setRunAllSortsName("Optimized Mid Singularity Quick Sort");
        this.setRunSortName("Optimized Mid Singularity Quicksort");
    ...
    void innerSort(int[] array, int[] buf, int a, int b, int depth, int rep) {
        while (b - a > insertlimit) {
            ...
            if (rIdx >= b)
                return;
            int[] pr = partition(array, buf, a, b, array[a + (rIdx - 1 - a) / 2]);
            ...
            depth++;
            if (rLen < lLen) {
                innerSort(array, buf, pr[1], b, depth, rep);
                b = pr[0];
            } else {
                innerSort(array, buf, a, pr[0], depth, rep);
                a = pr[1];
            }
        }
