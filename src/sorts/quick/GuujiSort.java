...
public final class GuujiSort extends Sort {
    ...
    protected int[] partition(int[] array, int[] aux, int a, int b, int bLen, int piv, boolean pCmp,
        ...
        Highlights.clearMark(2);
        if (b - a <= bLen) return partEasy(array, aux, a, b, piv, pCmp);
        ...
        for (int i = a; i < b; i++) {
            ...
            if (hiPart ^ invert) {
                ...
                if (tr >= bLen) {
                    ...
                    tlStreaks = 0;
                    int tl = hiPart ? l : r;
                    if (badImbalance) swapCopy(array, t, aux, 0, tl, bLen, 1, true, false);
                    else {
    ...
    protected void sortHelper(int[] array, int[] buf, int a, int b, int bLen, boolean bad) {
        while (b - a > 32) {
            ...
            if (m == b) { // pivot is highest rank
                // partition again with inverted bias
