...
public final class IndexSmartSingularityQuickSort extends Sort {
    ...
    void innerSort(int[] array, int[] idx, int a, int b, int depth, int rep) {
        while (b - a > insertlimit) {
            ...
            depth++;
            if (rLen < lLen) {
                innerSort(array, idx, pr[1], b, depth, rep);
                b = pr[0];
            } else {
                innerSort(array, idx, a, pr[0], depth, rep);
                a = pr[1];
            }
        }
