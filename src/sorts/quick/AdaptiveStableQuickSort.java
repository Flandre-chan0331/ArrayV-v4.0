...
public class AdaptiveStableQuickSort extends Sort {

    public AdaptiveStableQuickSort(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        this.setSortListName("Adaptive Stable Quick");
        this.setRunAllSortsName("Adaptive Stable Quick Sort");
        this.setRunSortName("Adaptive Stable Quicksort");
    ...
    void sortHelper(int[] array, int[] buf, int a, int b, boolean bad) {
        while (b - a > insertThreshold) {
            ...
            if (m == a) {
                ...
                bad = l < r / 8 || r < l / 8;
                if (r < l) {
                    sortHelper(array, buf, m, b, bad);
                    b = m;
                } else {
                    sortHelper(array, buf, a, m, bad);
                    a = m;
                }
            }
        }
