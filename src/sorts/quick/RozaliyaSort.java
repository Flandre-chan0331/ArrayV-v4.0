...
public final class RozaliyaSort extends Sort {
    ...
    public RozaliyaSort(ArrayVisualizer arrayVisualizer) {
        ...
        this.setSortListName("Rozaliya");
        this.setRunAllSortsName("Rozaliya Sort");
        this.setRunSortName("Rozaliya sort");
    ...
    protected int[] partition(int[] array, int[] buf, int a, int b, int piv1, int piv2, boolean bias) {
        Highlights.clearMark(2);
        int[] ptrs = new int[4];
        for (int i = a; i < b; i++) {
    ...
    protected void sortHelper(int[] array, int[] buf, int left, int right) {
        ...
        while (queue.size() > 0) {
            ...
            pr = partition(array, buf, a, b, piv1, piv2, !ternary);
            int m1 = pr[0], m2 = pr[1];
