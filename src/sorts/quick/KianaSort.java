...
Original name of this algorithm: Lazy Priority Logsort
...
public final class KianaSort extends Sort {
    ...
    public KianaSort(ArrayVisualizer arrayVisualizer) {
        ...
        this.setSortListName("Kiana");
        this.setRunAllSortsName("Kiana Sort");
        this.setRunSortName("Kianasort");
    ...
    protected boolean pivCmp(int v, int piv, boolean eqLower) {
    ...
    protected void sortHelper(int[] array, int left, int right, int bLen) {
        PriorityQueue<Partition> queue = new PriorityQueue<>((right - left - 1) / this.threshold + 1);
        queue.offer(new Partition(left, right, false, false));
        while (queue.size() > 0) {
    ...
    public void quickSort(int[] array, int a, int b) {
        ...
        if (b - a > threshold) sortHelper(array, a, b, productLog(b - a));
        else insertSort(array, a, b);
    }
