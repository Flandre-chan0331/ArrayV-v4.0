...
public final class IndexFaruSort extends Sort {
    ...
    public IndexFaruSort(ArrayVisualizer arrayVisualizer) {
        ...
        this.setSortListName("Index Faru");
        this.setRunAllSortsName("Index Faru Sort");
        this.setRunSortName("Index Farusort");
    ...
    int[] medOf5(int[] array, int[] indices) {
        if (indices.length < 5) return null; // we can use a sentinel value here
        for (int i = 0; i < 5; i++) {
            for(int j = i; j > 0; j--) {
    ...
    int pivCmp(int v, int piv1, int piv2, boolean bias) {
    ...
    protected int[] partition(int[] array, int[] idx, int a, int b, int piv1, int piv2, boolean bias) {
        Highlights.clearMark(2);
        int[] ptrs = new int[4];
        for (int i = a; i < b; i++) {
            Highlights.markArray(1, i);
    ...
    void consumePartition(int[] array, PriorityQueue<Partition> queue, int a, int b, boolean bad) {
        if (b - a > threshold)
            queue.offer(new Partition(a, b, bad));
        else
            insertSort(array, a, b);
    }
    
    protected void sortHelper(int[] array, int[] idx, int left, int right) {
        ...
        while (queue.size() > 0) {
            ...
            if (!ternary)
                consumePartition(array, queue, m1, m2, bad);
