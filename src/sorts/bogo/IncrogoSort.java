...
public final class IncrogoSort extends BogoSorting {
    ...
    public void runSort(int[] array, int sortLength, int bucketCount) {
        ...
        for (int i = 0; i < sortLength; i++) Writes.write(array, i, min, delay, true, false);
        while (!finalized) {
            int j = BogoSorting.randInt(0, sortLength);
            if (array[j] == max) Writes.write(array, j, min, this.delay, true, false);
            else Writes.write(array, j, array[j] + 1, this.delay, true, false);
